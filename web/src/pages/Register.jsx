...
export default function Register() {
  ...
  const getPasswordStrength = (pass) => {
    ...
    if (score === 1) return { score: 1, label: 'Weak', color: 'bg-[#FEF3C7] text-[#D97706]' }
    if (score === 2) return { score: 2, label: 'Moderate', color: 'bg-[#B7C99A] text-[#2D3D12]' }
    return { score: 3, label: 'Strong', color: 'bg-[#ECFDF5] text-[#059669]' }
  }
    ...
    <div className="flex items-center justify-center min-h-[calc(100vh-8rem)] px-4 py-12 bg-[#FAFBF8]">
      <div className="w-full max-w-md bg-white border border-[#E5E7EB] rounded-xl p-8 shadow-sm">
        <div className="mb-6 text-center space-y-2">
          ...
          <h1 className="text-2xl font-extrabold text-[#2F2F2F] tracking-tight">Create your account</h1>
          <p className="text-xs text-[#666666] font-medium">Join to monitor animal intrusion alerts</p>
        </div>
          ...
          <div>
            ...
            {password && (
              <div className="mt-2 space-y-1">
                <div className="flex justify-between items-center text-[11px] font-semibold text-[#666666]">
                  <span>Password strength</span>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${strength.color}`}>{strength.label}</span>
                </div>
              </div>
            )}
          </div>
        ...
        <p className="text-center text-xs font-medium text-[#666666] mt-6">
          ...
          <Link to="/login" className="font-bold text-[#6B8E23] hover:underline">
            Sign in
          </Link>
        </p>
      </div>
